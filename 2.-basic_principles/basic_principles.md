...
We have to make sure that everything we write, Python understands. Python understands any number you type
in directly, as well as common arithmetic operations such as `*` and `+` as it understood exactly what you
meant when you wrote `2*3`. it also has some built in functions it understands such as `print()`. We will
learn some more of these in built functions soon. It also understands `True` and `False` mean something.
`not` can operate on `True` and `False`. For working with words (in coding we call these strings), we have
to include quotation marks. Try running the following code without the quotations:
...
Note, the reason we have the comma between our string and our type command will be apparent later, its just a way to print two things with one command. Instead of typing `print("word type")` and then print `type(word)`.
...
`True` and `False` are of the type `Boolean` named after George Boole who worked a lot with the algebra you can do with `True` and `False` values.
The `print` keyword is of type `builtin_function` since it is a native function in Python. (as opposed to functions that we will write).
