# Project: Spring Calculator With CLI and Data Output

In this project, you will write a Python script that computes useful quantities
for a spring and saves the results to a plain text data file.
...
4. Save the results to a `.dat` file.
...
def parse_arguments():
    ...
    parser.add_argument("--output", default="spring_results.dat",
...
```bash
python3 spring_project.py --spring-constant 20 --mass 0.5 --displacements -0.2 -0.1 0.0 0.1 0.2 --output spring_results.dat
```
...
## Write the Data Output File

Add this function:

```python
def write_data_file(filename, k, mass, period, results):
    with open(filename, "w", encoding="utf-8") as file:
        file.write("# Spring calculation results\n")
        file.write(f"# spring_constant_N_per_m {k}\n")
        file.write(f"# mass_kg {mass}\n")
        file.write(f"# period_s {period}\n")
...
The lines that start with `#` are comments. They store useful information for
a human reader. The final lines form a table that another program can read
later.
...
def main():
    ...
    write_data_file(args.output, k, mass, period, results)
...
mass = 0.5 kg
period = 0.993 s
wrote output to spring_results.dat
```

Open `spring_results.dat`. It should contain:

```text
# Spring calculation results
# spring_constant_N_per_m 20.0
# mass_kg 0.5
# period_s 0.9934588265796101
...
6. Check that the table header is:
...
1. Round the data output values to 3 decimal places.
...
4. Add the average energy to the comment section of the `.dat` file.
5. Add a command-line option named `--label` and save it as a comment in the
   `.dat` file.
